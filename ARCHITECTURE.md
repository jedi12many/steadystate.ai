...
Both are **departures from steady state**, and *reasoning about any departure* — which ones matter, why, and what to do — is the product. Drift is the first-class signal and where the engine began; **malfunction** is the second (the `Symptom` type, §4; designed, roadmap §11). The name was always *steadystate*, not *driftfinder*: a problem that never touched your config is still a problem with your system, and blinding ourselves to it to protect a definition would serve the definition, not the operator.
...
DECLARED state (plugins — must be EASY)        OBSERVED state (rented)
 ...
 ansible · helm · k8s · pulumi                   each tool's own diff where possible
   ...
   Departures from steady state — the inputs the engine reasons about (one normalized shape):
     ...
     SYMPTOM  observed unhealthy right now       (health observers — crashloop/…)    [designed §4/§11]
        ▼
...
- **Symptom** *(designed — roadmap §11)* — an operational malfunction of a *declared* resource, observed now: `{ identity, kind, category (CrashLoopBackOff / Restarting / Unhealthy / Exited …), severity, evidence (last log line, restart count), provenance, detected_at }`. The parallel to Drift: where Drift says *config diverged*, Symptom says *config is fine but it's failing*.
...
*Designed (roadmap §11):* a **health observer** — the producer of `Symptom`s (§4), the operational counterpart to a StateSource. Where a StateSource reconciles declared vs observed *config* into Drift, a health observer reads the live *health* of declared resources into Symptoms (reusing the very detection the `kubectl`/`docker` enrichers already have, promoted from "escalate a drift" to "originate a Symptom"). Same access the source uses; detection rented, reasoning ours. This is the seam that lets the engine see malfunction with no drift, without becoming a monitor. **It largely subsumes the `kubectl`/`docker` enrichers:** once a Symptom is a first-class peer, a Symptom + a co-located Drift *correlate* into one root-caused Alert automatically — which is what "the enricher escalates a drift" was hand-rolling, but stronger (the symptom is evidence in the root cause, not just a severity bump). The enrichers' container-health detection graduates into the observer; `--enrich prometheus` (metric-threshold escalation, a different shape) stays.
...
2. **Summon — observe on demand** *(designed, roadmap §11).* `@steadystate observe <target>` dispatches a scan/observe of a named target *right now*, **regardless of the schedule** — someone just pinged you about prod, so you send the on-call agent to look rather than SSH-ing in yourself. It runs the full pipeline (drift + Symptoms + correlation) and posts the result back to the thread. This is the operator-**initiated** counterpart to the scheduled run; scheduling itself stays rented (cron / CI / a CronJob), and chat is the out-of-band trigger *and* the result surface. A `target` is a named, pre-registered scan config (source + access + `--label`), so `observe my_cluster` resolves to "k8s, this kubeconfig, env=prod-k8s."
...
**The inbound seam carries commands, not just approvals.** The same `INBOUND` adapters (§6) that today parse an approve/decline now normalize an operator message to a **Command** (`approve | decline | observe <target> | …`); `listen` becomes a ChatOps command surface, not only an approval listener.
...
12. Live-health enrichers (`--enrich prometheus | kubectl | docker`) — correlate a drift with the operational state of its resource (CrashLoopBackOff / restarts / unhealthy container + the failing pod/container's last log line), escalating *"failing since it drifted."* These are the detection that the designed `Symptom` work (below) promotes from "escalate" to "originate."

**Next:**
- **Operational malfunction as a first-class departure** — the thesis evolution committed in §1/§4: add the `Symptom` type, a **health observer** seam (§6) that produces Symptoms for declared resources, flow them through the existing pipeline, and **correlate them with drift** (the root-cause Alert no monitor produces). It largely subsumes the kubectl/docker enrichers (§6). Build pending; the design is locked here. Scope guardrails: declared resources only, detection rented.
- **Summon — chat-triggered observe** (§7): generalize the `INBOUND` adapters from parsing approvals to parsing Commands, so `@steadystate observe <target>` dispatches an on-demand scan/observe of a named target and posts the result to the thread — the operator-initiated counterpart to the scheduled run. Needs a small registry of named targets (name → source + access + label).
