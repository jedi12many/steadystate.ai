...
15. **Chat is two-way — the inbound `Command` seam + Summon** (§7): the `INBOUND` adapters parse a provider-agnostic `Command` (`verb + actor + argument`) over one shared grammar, so the listener takes `help` + `pending` (read-only discovery) and **`probe <target>`** (Summon — an on-demand scan of a named target from `STEADYSTATE_TARGETS`, read-only) alongside `approve`/`decline`, across all three providers. Summon runs through the shared `engine.build_report` the `scan` CLI uses, so there's one reasoning path, not two. The persistent listener ships as a Deployment ([deploy/kubernetes/listener.yaml](deploy/kubernetes/listener.yaml)) — the long-lived counterpart to the scheduled CronJob.

**Next:**
- **Async summons:** a slow scan (terraform plan) can exceed a chat provider's interaction timeout; defer (ack now, post the result when it's ready) so heavy targets aren't limited to the fast path.
