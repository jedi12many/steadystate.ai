...
3. **Surface** — push Alerts out, and take operator input back. Outbound: `console · slack · teams · prometheus · grafana` (`SURFACES`). Inbound has its own registry (`INBOUND`, mirroring `SURFACES`) over a signed HTTP listener — `listen --from <channel>` — so accepting approvals from a new chat provider (Slack today; Discord/Teams/email next) is an adapter (verify/handshake/parse/respond), not a fork of the listener.
...
**Next:** more inbound adapters (Discord, Teams, email) on the new seam · a remediation **audit log** · a Kubernetes security pack · more sources (Pulumi, Helm) · third-party plugin discovery (importlib entry points).
