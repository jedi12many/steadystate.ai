...
- Three-tier reasoning (Signal -> Event -> Alert) + Brain Tuning: every drift is a Signal (counted); filtering records Events, and analysis + correlation raise Alerts (surfaced, with or without a recommended action). A single `--tuning lenient|default|strict` knob moves the tiers together, and the LLM analyst runs only at the Alert tier. Console shows the full breakdown; Slack/Teams page on Alerts only.
