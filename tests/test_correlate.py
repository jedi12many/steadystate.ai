...
def test_correlate_groups_by_cause(monkeypatch):
    monkeypatch.setattr(LLMAnalyst, "_complete", lambda self, system, user: _GROUPED)
    clusters = LLMAnalyst().correlate([_drift("a"), _drift("b")])
...
def test_correlate_rejects_incomplete_coverage(monkeypatch):
    ...
    monkeypatch.setattr(LLMAnalyst, "_complete", lambda self, system, user: bad)
    clusters = LLMAnalyst().correlate([_drift("a"), _drift("b")])
...
def test_correlate_handles_non_json(monkeypatch):
    monkeypatch.setattr(LLMAnalyst, "_complete", lambda self, system, user: "sorry, no idea")
