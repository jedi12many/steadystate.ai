...
from .github_pr import GitHubPRDelivery
from .patch_file import PatchFileDelivery

# name -> zero-arg factory -> DeliveryAdapter. patch-file is the auth-free floor; github-pr holds
# its own token. A gitlab-mr / bitbucket adapter slots in here behind the same seam.
_BUILTIN_DELIVERIES: dict[str, Callable[[], DeliveryAdapter]] = {
    "patch-file": PatchFileDelivery,
    "github-pr": GitHubPRDelivery,
}
