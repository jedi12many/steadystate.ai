"""patch-file delivery (Level 0): write the artifact's diff to a ``.patch`` on disk.

The auth-free floor. No git, no remote, no credentials -- it just writes the unified diff, and a
human (or a downstream CI step) runs ``git apply``. Works everywhere, including GitHub EMU where
a tool can't authenticate as a person. Every higher rung (branch, PR) wraps this same patch.
"""
