...
| `STEADYSTATE_TARGETS` | `steadystate/targets.json` | The named-targets registry a scan / chat / MCP server resolves. Targets are **intent** once their creds are brokered (pointers, never keys), so the default is the **committed** `steadystate/` (undotted) like checks/solutions — every entry point finds it with no env var (a client-spawned MCP server included) — falling back to the legacy gitignored `.steadystate/targets.json` if a repo has one. Splitting it per folder is how you **wall** environments. A target may set **`kubeconfig_from`** — a broker command (e.g. `akeyless get-secret-value …`, `vault kv get …`) whose stdout is a **fresh kubeconfig, minted at probe time** and deleted when the probe finishes — so a long-running `up`/`mcp` never holds an expired or standing credential. Argv, no shell; fail-closed; see [examples/brokered-creds](./examples/brokered-creds/). |
...
| `STEADYSTATE_REQUESTS` | `steadystate/requests.json` | The committed **request recipes** `request <name>` fulfills: each is a vetted ask → a deterministic one-line edit → a **review-gated PR** in the repo that owns the decision (e.g. add a domain to the proxy allowlist). Parameters are regex-validated; the model never composes a diff; deduped per (recipe, values); audited. See [examples/requests](./examples/requests/). |
| `STEADYSTATE_MUTES` | `steadystate/mutes.json` | The **committed mutes** — a mute is a *decision* ("this fingerprint is benign"), not runtime state, and fingerprints are content-derived, so the committed file **re-applies on any state db's first scan** (a rebuilt host, a fresh wall). Promote with `mute --commit` (one) or `commit-mutes` (export the db's permanent mutes); each entry carries the finding's title + who muted, so the PR review means something. `unmute` warns when a mute is committed (the file, not the db, is then the thing to change). Snoozes are temporal and stay db-local. |
