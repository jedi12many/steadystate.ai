...
- **Terminal** — `steadystate health` for the working/degraded/down verdict; `summary` for the
  one-glance rollup; `findings` / `show` to inspect; **`analyze <fp>`** for a root-cause analysis of a
  crash/panic — we capture the **logs leading up to the failure** (the cause lives in the lead-up, not
  the panic line) and let the model **investigate** them like an on-call engineer, quoting what it
  cites and labelling inference so it stays checkable; **`watch <target>`** to catch a transient
