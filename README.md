...
- **Two ways to fix, picked at review** — a `suggest` suggestion carries both directions where they exist: the **enforce** command `approve` would run (change reality to match config), *and* an **accept-reality patch** — a reviewable code change (change config to match reality), computed **deterministically** (the model never authors it). The first patch case is a Terraform `REMOVED` drift, where enforcing would *destroy* the resource: instead, the patch **re-adds the deleted declaration** so the destroy is averted. `steadystate pending` shows the patch; you `git apply` + merge it — the tool never applies a code change itself, so it works even where automation can't authenticate as a person (e.g. GitHub EMU).
